...
- Inference request content is sent only to the provider the user selects (for example OpenAI, Anthropic, OpenRouter, Vercel AI Gateway, local Ollama, on-device browser AI, or a user-configured OpenAI-compatible server).
...
| Data | Purpose | Where |
| --- | --- | --- |
| Provider API keys (OpenAI, Anthropic, OpenRouter, Vercel AI Gateway, optional Ollama account key for web search, optional keys for custom endpoints) | Authenticate requests to the selected provider | `chrome.storage.local` |
...
- **Vercel AI Gateway:** chat messages and the stored Vercel AI Gateway API key are sent to `https://ai-gateway.vercel.sh` for the selected model. Hosted `{ type: "web_search" }` is executed by the Gateway (Perplexity). The public model catalog (`GET /v1/models`) is fetched without an API key to populate the Options UI.
...
| `https://ai-gateway.vercel.sh/*` | Call the Vercel AI Gateway models catalog and Chat Completions API |
