...
- Inference request content is sent only to the provider the user selects (for example OpenAI, OpenRouter, local Ollama, or a user-configured OpenAI-compatible server).
...
| Data | Purpose | Where |
| --- | --- | --- |
| Provider API keys (OpenAI, OpenRouter, optional keys for custom endpoints) | Authenticate requests to the selected provider | `chrome.storage.local` |
